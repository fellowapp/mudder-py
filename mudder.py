...
class SymbolTable:
	def __init__(self, symbols: Iterable[str], symbol_map: Optional[Dict[str, int]] = None):
		...
		for i in range(len(symbols)):
			if i not in symbol_values:
				raise ValueError(f"{len(symbols)} symbols given but {i} not found in symbol table")
	...
	def string_to_digits(self, string: Iterable[str]) -> List[int]:
		if isinstance(string, str):
			if not self.is_prefix_code:
				raise ValueError("Parsing without prefix code is unsupported. Pass in array of stringy symbols?")
			string = (c for c in string if c in self.sym2num)

		return [self.sym2num[c] for c in string]

	def digits_to_number(self, digits: Reversible[int], base: Optional[int] = None) -> int:
		base = base or self.max_base
		current_base = 1
		accum = 0
	...
	def round_fraction(self, numerator: int, denominator: int, base: Optional[int] = None) -> List[int]:
		base = base or self.max_base
	...
	def mudder(
		...
		final_digits = [v + self.round_fraction(rem, den, base) for v, rem, den in intermediate_digits]
		final_digits.insert(0, ad)
		final_digits.append(bd)
		return [self.digits_to_string(v) for v in chop_successive_digits(final_digits)[1:-1]]


def long_div(numerator: List[int], denominator: int, base: int) -> Tuple[List[int], int]:
	result = []
	remainder = 0
...
def long_sub_same_len(  # noqa: C901
	a: List[int], b: List[int], base: int, remainder: Optional[Tuple[int, int]] = None, denominator=0,
) -> Tuple[List[int], int]:
	if len(a) != len(b):
	...
	for i in reversed(range(len(a))):
		...
		for j in reversed(range(i)):
			if a[j] > 0:
				...
				ret[i] = a[i] + (denominator if remainder and i == len(a) - 1 else base) - b[i]
				do_break = True
				break
		if do_break:
			continue
...
def long_linspace(a: List[int], b: List[int], base: int, n: int, m: int) -> List[Tuple[List[int], int, int]]:
	if len(a) < len(b):
		a = right_pad(a, len(b))
	elif len(b) < len(a):
		b = right_pad(b, len(a))
	...
	for _i in range(1, n + 1):
		sum_, _carry, rem, den = long_add_same_len(a_prev, b_prev, base, a_prev_rem + b_prev_rem, m)
		ret.append((sum_, rem, den))
		a_prev, a_prev_rem = long_sub_same_len(a_prev, a_div, base, (a_prev_rem, a_div_rem), m)
		b_prev, _carry, b_prev_rem, _den = long_add_same_len(b_prev, b_div, base, b_prev_rem + b_div_rem, m)
	return ret
...
alphabet = SymbolTable(alpha_lower, dict(zip(alpha_lower + alpha_upper, cycle(range(26)))))
